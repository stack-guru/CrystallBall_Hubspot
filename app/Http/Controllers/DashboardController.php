...
class DashboardController extends Controller
{
    ...
    public function annotationsMetricsDimensionsIndex(Request $request)
    {
        ...
        $annotations = DB::select("SELECT T2.event_name, T2.category, T2.show_at, T3.* FROM ($annotationsQuery) AS T2 INNER JOIN (
            SELECT DATE_SUB(statistics_date, INTERVAL 7 DAY) as seven_day_old_date, SUM(sessions_count) as sum_sessions_count, SUM(events_count) as sum_events_count  FROM google_analytics_metric_dimensions
