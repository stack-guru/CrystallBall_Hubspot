...
class AnnotationQueryHelper
{
    ...
    {
        $annotationsQuery = "";
        $annotationsQuery .= "SELECT DISTINCT `annotations`.`is_enabled`, DATE(`show_at`) AS show_at, `annotations`.`id`, `category`, `event_name`, `url`, `description` FROM `annotations`";
        if ($googleAnalyticsPropertyId && $googleAnalyticsPropertyId !== '*') {
    ...
    {
        $annotationsQuery = "";
        $annotationsQuery .= "select 1, update_date AS show_at, google_algorithm_updates.id, category, event_name, NULL as url, description from `google_algorithm_updates`";
    ...
    {
        return "select 1, show_at, id, category, event_name, url, description from `web_monitor_annotations` WHERE `web_monitor_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }
    ...
    public static function holidaysQuery(User $user, string $googleAnalyticsPropertyId)
    {
        ...
        return "select 1, holiday_date AS show_at, holidays.id, category, event_name, NULL as url, description from `holidays` inner join `user_data_sources` as `uds` on `uds`.`country_name` = `holidays`.`country_name` where $gAPropertyCriteria AND (`uds`.`user_id` = " . $user->id . " and `uds`.`ds_code` = 'holidays')";
    }

    public static function retailMarketingQuery(User $user, string $googleAnalyticsPropertyId)
    {
        $gAPropertyCriteria = self::googleAnalyticsPropertyWhereClause($googleAnalyticsPropertyId);
        return "select 1, show_at, retail_marketings.id, category, event_name, NULL as url, description from `retail_marketings` inner join `user_data_sources` as `uds` on `uds`.`retail_marketing_id` = `retail_marketings`.id where $gAPropertyCriteria AND (`uds`.`user_id` = " . $user->id . " and `uds`.`ds_code` = 'retail_marketings')";
    }

    public static function openWeatherMapQuery(User $user, string $googleAnalyticsPropertyId)
    {
        $gAPropertyCriteria = self::googleAnalyticsPropertyWhereClause($googleAnalyticsPropertyId);
        return "select 1, alert_date, open_weather_map_alerts.id, open_weather_map_cities.name, description, null, description from `open_weather_map_alerts` inner join `user_data_sources` as `uds` on `uds`.`open_weather_map_city_id` = `open_weather_map_alerts`.open_weather_map_city_id inner join `user_data_sources` as `owmes` on `owmes`.`open_weather_map_event` = `open_weather_map_alerts`.`event` inner join `open_weather_map_cities` on `open_weather_map_cities`.id = `open_weather_map_alerts`.`open_weather_map_city_id` where $gAPropertyCriteria AND (`uds`.`user_id` = " . $user->id . " and `uds`.`ds_code` = 'open_weather_map_cities')";
    }

    public static function googleAlertsQuery(User $user, string $googleAnalyticsPropertyId)
    {
        $gAPropertyCriteria = self::googleAnalyticsPropertyWhereClause($googleAnalyticsPropertyId);
        return "select 1, alert_date, google_alerts.id, 'News Alert', title, google_alerts.url, description from `google_alerts` inner join `user_data_sources` as `uds` on `uds`.`value` = `google_alerts`.tag_name where $gAPropertyCriteria AND (`uds`.`user_id` = " . $user->id . " and `uds`.`ds_code` = 'google_alert_keywords' AND DATE(google_alerts.created_at) > DATE(DATE_ADD(uds.created_at, INTERVAL 1 DAY)) )";
    }

    public static function wordPressQuery()
    {
        $annotationsQuery = "";
        $annotationsQuery .= "select 1, update_date, wordpress_updates.id, category, event_name, url, description from `wordpress_updates`";
    ...
    {
        return "select 1, created_at, null, category, event_name, url, description from `keyword_tracking_annotations` WHERE `keyword_tracking_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function facebookTrackingQuery(array $userIdsArray)
    {
        return "select 1, created_at, null, category, event_name, url, description from `facebook_tracking_annotations` WHERE `facebook_tracking_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function instagramTrackingQuery(array $userIdsArray)
    {
        return "select 1, created_at, null, category, event_name, url, description from `instagram_tracking_annotations` WHERE `instagram_tracking_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function twitterTrackingQuery(array $userIdsArray)
    {
        return "select 1, created_at, null, category, event_name, url, description from `twitter_tracking_annotations` WHERE `twitter_tracking_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function bitbucketCommitQuery(array $userIdsArray)
    {
        return "select 1, created_at, null, category, event_name, url, description from `bitbucket_commit_annotations` WHERE `bitbucket_commit_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function gitHubCommitQuery(array $userIdsArray)
    {
        return "select 1, created_at, null, category, event_name, url, description from `github_commit_annotations` WHERE `github_commit_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function googleAdsQuery(array $userIdsArray)
    {
        return "select 1, created_at, null, category, event_name, url, description from `google_ads_annotations` WHERE `google_ads_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }

    public static function applePodcastQuery(array $userIdsArray)
    {
        return "select 1, podcast_date, null, category, event, url, description from `apple_podcast_annotations` WHERE `apple_podcast_annotations`.`user_id` IN ('" . implode("', '", $userIdsArray) . "')";
    }
}
