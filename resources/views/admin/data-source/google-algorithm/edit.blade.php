    ...
    <div class="row ml-0 mr-0 justify-content-center">
        <div class="col-md-10 p-5">
            ...
            <form action="{{route('admin.data-source.google-algorithm-update.update', $googleAlgorithmUpdate->id)}}" method="post">
                ...
                <div class="form-group">
                    <label for="">URL</label>
                    <input type="text" name="url" id="url" value="{{ $googleAlgorithmUpdate->url }}"
                        class="form-control">
                    @error('url')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control" id="status">
                        <option value="confirmed" @if($googleAlgorithmUpdate->status == 'confirmed') selected @endif>Confirmed</option>
                        <option value="unconfirmed" @if($googleAlgorithmUpdate->status == 'unconfirmed') selected @endif>Unconfirmed</option>
                    </select>
                    @error('status')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
