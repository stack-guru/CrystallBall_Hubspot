                ...
                <tbody>
                  @foreach ($webMonitors as $webMonitor)
                    <tr>
                      ...
                      <td>{{ $webMonitor->last_synced_at ? $webMonitor->last_synced_at->format(config('app.format.datetime')) : "Not Synced" }}</td>
                      <td>
                        <form id="deleteUptimeRobotForm{{ $webMonitor->id }}" method="POST" action="{{ route('admin.web-monitor.destroy', $webMonitor->id) }}">
                          {{ csrf_field() }}
                          {{ method_field('DELETE') }}
                        </form>
                        <button type="button" onclick="if(confirm('Do you really want to delete this entity?')) document.getElementById('deleteUptimeRobotForm{{ $webMonitor->id }}').submit();" class="btn btn-danger m-2">Delete</button>
                      </td>
                    </tr>
                  @endforeach
                </tbody>
