                            ...
                            <table class="table table-hoved table-bordered" id="myTable">
                                ...
                                <tbody>
                                    @foreach ($users as $user)
                                        <tr>
                                            <td>{{ $user->team_name }}{{ $user->department ? ', ' . $user->department : '' }}
                                            </td>
                                            <td>
                                                {{ $user->email }}
                                                @if ($user->user)
                                            ...
                                            <td>
                                                ...
                                                <a class="btn btn-primary m-2"
                                                    ...
                                                    <button type="button"
                                                        onclick="if(confirm('Do you really want to delete this entity?')) document.getElementById('deleteUserForm{{ $user->id }}').submit();"
                                                        class="btn btn-danger m-2">Delete</button>

                                                    <form id="loginUserForm{{ $user->id }}" method="POST"
                                                        action="{{ route('admin.user.login', $user->id) }}">
                                                        @csrf
                                                    </form>
                                                    <button type="button"
                                                        onclick="document.getElementById('loginUserForm{{ $user->id }}').submit()"
                                                        class="btn btn-secondary m-2">Impersonate</button>

                                                    <form id="makeOwnerUserForm{{ $user->id }}" method="POST"
                                                        action="{{ route('admin.user.make-owner', $user->id) }}">
                                                        @csrf @method("PUT")
                                                    </form>
                                                    <button type="button"
                                                        onclick="document.getElementById('makeOwnerUserForm{{ $user->id }}').submit()"
