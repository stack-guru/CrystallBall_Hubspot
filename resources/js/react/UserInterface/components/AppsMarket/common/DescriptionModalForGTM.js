...
class DescriptionModalForGTM extends React.Component {
    constructor(props) {
        super(props);
    }
    ...
    showConfirm = () => {
        ...
        swal.fire({
            iconHtml: `<img src="/${(this.props.name || '').toLowerCase()}-small.svg">`,
            ...
            confirmButtonText: `<span class='text-white'>${ this.props.serviceName }</span>`,
            customClass: {
                htmlContainer: "py-3",
            },
            customClass: {
            ...
            if( result.isConfirmed ) {
                location.href = `/socialite/${ this.props.name.toLowerCase() }`;
                localStorage.setItem("repo", this.props.name);
            }
        });
    }
...
export default DescriptionModalForGTM;
